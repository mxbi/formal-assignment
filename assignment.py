...
class Assignment:
    def __init__(self, table_size, num_tables, num_nights=2):
    ...
    def run(self, groups, assignment_seed=42):
        ...
        for night, subgroups in groupby(groups_sorted_by_night, key=lambda x: x.dateprefs):
            ...
            try:
                tables = self.sort_into_tables(subgroups)
                ...
                tables = self.sort_into_tables(subgroups, extra_tables=1)
        ...
        night_a_tables = all_tables[:n_tables_per_formal]
        night_b_tables = all_tables[n_tables_per_formal:]
        ...
        # Final assignment :)
        return [night_a_selection, night_b_selection]
